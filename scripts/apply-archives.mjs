...
if (!file || file.startsWith('--')) {
  console.error('usage: node scripts/apply-archives.mjs <captures.txt> [--write]');
  process.exit(2);
}
...
for (const [i, line] of lines.entries()) {
  ...
  const archived = await resolve(rawArch, original);
