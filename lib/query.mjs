...
export function run(records, { terms, free, raw = '' }) {
  ...
  const holds = (r, t) => {
    switch (t.kind) {
      case 'context': {
        ...
        return v != null && cmp(v, t.op, t.value);
      }
      case 'params': {
      ...
      case 'price': {
        ...
        return p != null && cmp(p, t.op, t.value);
      }
      ...
      case 'multimodal': {
        ...
        return Boolean(m) && (m.input.length > 1 || m.output.length > 1);
      }
      ...
      case 'capability':
        return displayTags(r).includes(t.value) || Boolean(r.capabilities?.includes(t.value));
      default: return true;
    }
  };

  const structured = records.filter((r) => [...byKind.entries()].every(([kind, list]) =>
    (OR_KINDS.has(kind) ? list.some((t) => holds(r, t)) : list.every((t) => holds(r, t)))));
  ...
  matched.sort((a, b) => (score(b) + textHit(b)) - (score(a) + textHit(a))
