...
export const COMPANY_SLUG = {
  ...
  // No mark, deliberately. lobe-icons has nothing for Ornith — all 903 of its
  // icons were checked — and Ornith's own logo is an illustrated mascot: a
  // crested bird in glasses and a hoodie, five colours, 748x240. This sprite is
  // monochrome at ~20px, so that cannot be reproduced in it.
  //
  // Two simplified silhouettes were drawn and rendered at 20/32/64/128px before
  // this line was written. Both read as a blob with ears rather than a bird. A
  // mark that misidentifies a lab is worse than no mark, so it renders as the
  // "Or" monogram until Ornith or lobe-icons publishes an SVG.
  Ornith: 'other', 'Ornith Team': 'other', DeepReinforce: 'other',

  Tencent: 'tencent', 'Tencent Hunyuan': 'tencent',

  // The company mark, not the model brand: the same choice as Alibaba, whose
  // records carry Alibaba's mark rather than Qwen's. Hunyuan's circular mark
  // exists in lobe-icons too, but the lab is Tencent.
  //
  // Xiaomi: no glyph, deliberately. lobe-icons' only Xiaomi entry is the
  // "Xiaomi MiMo" WORDMARK, and rendered at 16 and 20px — the sizes this
  // sprite is drawn at — it is an unreadable grey smudge. The monogram is
  // legible; a smudge is not. Revisit if lobe-icons publishes a glyph.
  Xiaomi: 'other', 'Xiaomi MiMo': 'other',

  // TypeSafe AI has no mark in lobe-icons at all.
  'TypeSafe AI': 'other', TypeSafe: 'other',
};
...
/** Total parameters written in the name — "27B", "2.4T" — never the active count. */
function sizeOf(record) {
  // (?<![a-z]) skips "A3B"-style active-parameter tokens and the "7B" of "8x7B".
  const m = /(?<![a-z])(\d+(?:\.\d+)?)([bmt])\b/.exec(String(record?.model ?? '').toLowerCase());
  return m ? Number(m[1]) * { b: 1e9, m: 1e6, t: 1e12 }[m[2]] : null;
}

/**
 * The version number in the name, as a DECIMAL.
 *
 * Not a tuple, on purpose: xAI numbered Grok 4.20 in March and Grok 4.5 in
 * July, so as a tuple 4.20 outranks 4.5 and Grok 4.5 would lose its
 * predecessor. As decimals 4.2 < 4.5, which is also the order they shipped.
 */
function versionOf(record) {
  const name = String(record?.model ?? '').toLowerCase()
    .replace(/\([^)]*\)/g, ' ')
    .replace(/(?<![a-z])\d+(?:\.\d+)?\s*x\s*\d+(?:\.\d+)?b\b/g, ' ')
    .replace(/(?<![a-z])a?\d+(?:\.\d+)?[bmt]\b/g, ' ');
  const m = /v?(\d+(?:\.\d+)?)/.exec(name);
  return m ? parseFloat(m[1]) : null;
}

/**
 * Of the candidates, the ones nearest this release in size.
 *
 * Qwen ships one generation at several sizes, on different days. By date alone
 * the release before Qwen3.8-2.4T-A95B is Qwen3.8-27B, and "parameters 27B →
 * 2.4T" would be printed as a change when it is two different models. A 9B
 * follows a 9B. When the name carries no size, nothing is filtered.
 */
function nearestSize(record, candidates) {
  const size = sizeOf(record);
  if (size == null) return candidates;
  const sized = candidates.filter((r) => sizeOf(r) != null);
  if (!sized.length) return candidates;
  const gap = (r) => Math.abs(Math.log(sizeOf(r) / size));
  const best = Math.min(...sized.map(gap));
  // Nearest is not the same as near. When the only later Qwen release is 89x
  // larger, it is a different model, not the next step; the flagship line's
  // 397B -> 2.4T is 6x. Beyond 8x there is no neighbour at all.
  if (best > Math.log(8)) return [];
  return sized.filter((r) => gap(r) - best < 1e-9);
}

/**
 * The series a release belongs to inside its family: Opus, not Claude.
 *
 * A family is what a lab presents together — Claude holds Opus, Sonnet, Haiku,
 * Fable and Mythos — but lineage runs inside one of those lines. Ordering the
 * whole family by date made Claude Mythos 5.1 the "previous release" of Claude
 * Opus 5.5, and the page then diffed two different products and reported that
 * nothing was comparable. The useful question is what changed from Opus 5.
 *
 * DERIVED FROM THE NAME, because the name is how the lab states the series:
 * strip the version numbers, the size tokens and "preview", and what remains is
 * the line. "Claude 3 Opus" and "Claude Opus 4.5" both reduce to "claude opus";
 * "GPT-5.6 Sol" and "GPT-6 Sol" to "gpt sol"; "Qwen3.8-27B" to "qwen". Word
 * order does not matter, so Anthropic's 2024 rename from "Claude 3.5 Sonnet" to
 * "Claude Sonnet 4.5" stays one series.
 *
 * Storing a `series` field would be a second copy of a fact the name already
 * holds, and CLAUDE.md is explicit about that.
 */
export function seriesOf(record) {
  return String(record?.model ?? '').toLowerCase()
    .replace(/\([^)]*\)/g, ' ')                                // parentheticals
    .replace(/\b\d+(?:\.\d+)?\s*x\s*\d+(?:\.\d+)?b\b/g, ' ')       // 8x7B
    .replace(/\ba?\d+(?:\.\d+)?[bmt]\b/g, ' ')                // 27B, A3B, 2.4T — sizes, not versions
    .replace(/\b(?:preview|exp|experimental|latest)\b/g, ' ')
    .replace(/v?\d+(?:\.\d+)*/g, ' ')                          // versions, attached or not: Qwen3.8, Hy3, K2
    .replace(/[^a-z]+/g, ' ')
    .trim()
    .split(' ')
    .filter(Boolean)
 ...
 * corrected.
 *
 * PREDECESSOR AND SUCCESSOR COME FROM THE SAME SERIES (seriesOf). A family
 * holds several lines, and "the release before this one" means before it in
 * this line: Opus 5.5 follows Opus 5, not the Mythos model Anthropic shipped
 * three weeks earlier. The first release of a series has no predecessor, and
 * that is reported as none rather than borrowed from a neighbouring line.
 *
 * SIBLINGS COME FROM THE WHOLE FAMILY. A launch day is a family event — GPT-5.6
 * Sol, Luna and Terra shipped together as three sizes — so "released alongside"
 * spans series while "after" does not. Anything sharing a date is a sibling and
 * never a predecessor or successor.
 *
 * Every comparison the build makes — the Lineage row, the model page's What
 * changed, the family rail and the family's What changed — asks this function,
 * so no two of them can pick different neighbours. They used to: the model
 * page took the previous record by date, which diffed Luna against Sol.
 *
...
export function lineageOf(record, all) {
  ...
  const line = seriesOf(record);
  const same = kin.filter((r) => seriesOf(r) === line);
  const v = versionOf(record);

  // A release numbered ABOVE this one cannot have come before it in the same
  // line, and one numbered below cannot come after. Qwen-Image-2.1 shipped after
  // Qwen-Image-3.0 because they are two lines (open weights and API) sharing a
  // name; without this, 2.1 read as 3.0's successor.
  const notAhead = (r) => v == null || versionOf(r) == null || versionOf(r) <= v;
  const notBehind = (r) => v == null || versionOf(r) == null || versionOf(r) >= v;

  const before = nearestSize(record, same.filter((r) => canonicalDate(r) < on && notAhead(r)))
    .sort((a, b) => canonicalDate(b).localeCompare(canonicalDate(a)));
  const after = nearestSize(record, same.filter((r) => canonicalDate(r) > on && notBehind(r)))
