...
export const COMPANY_SLUG = {
  ...
  // No mark: lobe-icons carries nothing for Ornith, whose GitHub org was created
  // in July 2025, and this project inlines only icons it can license (MIT). The
  // entry is explicit rather than left to fall through, because the smoke test
  // demands a decision per company — a silent fallback is how a lab ends up
  // rendering as a monogram nobody chose. It renders as the "Or" monogram until
  // a licensed mark exists.
